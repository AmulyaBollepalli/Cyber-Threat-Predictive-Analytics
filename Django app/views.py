...
def predict(request):
    ...
    # Load the pipeline (scaler + model) from the file
    with open(PROJECT_PATH+'/model/XGBoost.pkl', 'rb') as file:
        loaded_pipeline = pickle.load(file)

    # Reshape the new test data to match the input shape expected by the model
    X_new = sample_input_record.reshape(1, -1)  # Reshape the input to 2D (1 sample, n features)

    # Use the loaded pipeline to predict on new, unseen data
    y_pred = loaded_pipeline.predict(X_new)

    # Print or analyze the prediction results
